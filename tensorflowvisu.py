...
def tf_format_mnist_images(X, Y, Y_, n=100, lines=10):
    ...
    everything_incorrect_first = tf.concat(0, [incorrectly_recognised_indices,
                                               correctly_recognised_indices])  # images reordered with indeces of unrecognised images first
    ...
    superimposed_digits = tf.select(correct_prediction_s, tf.zeros_like(correct_tags),
    ...
    recognised_bkg = tf.gather(tf.concat(0, [incorrect_bkg, correct_bkg]), tf.cast(correct_prediction_s,
